...
![清单 2.1 第 2 章中示例的 Gradle build.gradle.kts](Chapter2-Verticles.assets/Listing_2_1.png)
...
![清单 2.2 减少 Netty 冗长的 Logback 配置](Chapter2-Verticles.assets/Listing_2_2.png)
...
>
> **💡提示:** Vert.x使用Netty，使用默认的Logback配置登录Netty非常繁琐。我们可以通过创建一个`src/main/resources/logback.xml`文件并添加如 清单2.2 所示的配置来减少日志条目的数量。为了使本书中的日志示例更短，我还删除了事件日期并缩短了日志程序类名($logger{0})。请参考Logback文档了解如何配置它(https://logback.qos.ch/manual/index.html)

### 2.1.2 verticle类

整个verticle和应用程序都适合下面的Java类:

![清单 2.3 一个verticle示例](Chapter2-Verticles.assets/Listing_2_3.png)
...
![清单 2.4 运行 HelloVerticle 时的示例日志输出](Chapter2-Verticles.assets/Listing_2_4.png)
...
![图 2.1 清单 2.3 的执行](Chapter2-Verticles.assets/Figure_2_1.png)
...
![清单 2.5 一个事件循环被阻塞的例子](Chapter2-Verticles.assets/Listing_2_5.png)
...
![清单 2.6 运行清单 2.5 时的日志输出](Chapter2-Verticles.assets/Listing_2_6.png)
...
![清单 2.7 异步启动生命周期方法的示例](Chapter2-Verticles.assets/Listing_2_7.png)
...
![图 2.2 使用 Promise 和监听处理器启动 HTTP 服务器的时序图](Chapter2-Verticles.assets/Figure_2_2.png)
...
![图 2.3 在没有 Promise 和监听处理器的情况下启动 HTTP 服务器的时序图](Chapter2-Verticles.assets/Figure_2_3.png)
...
![清单 2.8 要部署的示例 Verticle](Chapter2-Verticles.assets/Listing_2_8.png)
...
![清单 2.9 部署和取消部署其他verticles的一个verticle ](Chapter2-Verticles.assets/Listing_2_9.png)
...
![清单 2.10 部署 Deployer verticle 的主类](Chapter2-Verticles.assets/Listing_2_10.png)

最后但同样重要的是，`Deployer` verticle 本身可以从主方法和类部署，如**清单 2.10** 所示。 运行此示例会产生类似于以下清单中的日志条目。

![清单 2.11 运行清单 2.10 中代码的日志摘录](Chapter2-Verticles.assets/Listing_2_11.png)
...
![清单 2.12 将配置数据传递给 Verticle](Chapter2-Verticles.assets/Listing_2_12.png)
...
![清单 2.13 运行清单 2.12 中的代码时的示例执行输出](Chapter2-Verticles.assets/Listing_2_13.png)
...
![清单 2.14 一个示例 worker verticle](Chapter2-Verticles.assets/Listing_2_14.png)
...
![清单 2.15 运行清单 2.14 的示例输出](Chapter2-Verticles.assets/Listing_2_15.png)
...
![图 2.4 executeBlocking 调用中的交互](Chapter2-Verticles.assets/Figure_2_4.png)

以下清单显示了一个示例用法。

![清单 2.16 使用 executeBlocking](Chapter2-Verticles.assets/Listing_2_16.png)
...
![清单 2.17 运行清单 2.16 时的示例输出](Chapter2-Verticles.assets/Listing_2_17.png)
...
![图 2.5 一个事件循环 Verticle 及其环境](Chapter2-Verticles.assets/Figure_2_5.png)
...
![图 2.6 一个 worker Verticle 及其环境](Chapter2-Verticles.assets/Figure_2_6.png)
...
![清单 2.18 创建没有 Verticle 的上下文](Chapter2-Verticles.assets/Listing_2_18.png)

正如您在下一个清单中看到的，每个上下文都被分配给一个事件循环。

![清单 2.19 运行清单 2.18 的示例输出](Chapter2-Verticles.assets/Listing_2_19.png)

上下文对象支持更多操作，例如保存上下文范围的任意键/值数据和声明异常处理程序。 下面的清单显示了一个示例，其中 `foo` 键包含字符串 `bar`，并且声明了一个异常处理程序以在事件循环线程上执行处理程序时捕获和处理异常。

![清单 2.20 使用上下文数据和异常处理](Chapter2-Verticles.assets/Listing_2_20.png)
...
![清单 2.21 运行清单 2.20 的示例输出](Chapter2-Verticles.assets/Listing_2_21.png)
...
![清单 2.22 混合不同的线程模型](Chapter2-Verticles.assets/Listing_2_22.png)
...
![清单 2.23 运行清单 2.22 时的示例输出](Chapter2-Verticles.assets/Listing_2_23.png)
