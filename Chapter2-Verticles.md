...
**配置 Vert.x 阻塞线程检查器**
默认情况下，阻塞线程检查器抱怨之前的时间限制是两秒，但可以将其配置为不同的值。 有些环境，例如嵌入式设备，处理能力较慢，增加它们的线程检查器阈值是正常的。

您可以使用系统属性来更改设置：
+ `-Dvertx.options.blockedThreadCheckInterval=5000` 将时间间隔更改为五秒。
+ `-Dvertx.threadChecks=false` 禁用线程检查器。

请注意，此配置是全局的，不能在每个verticle的基础上进行微调。
...
在清单 2.7 中，`Promise` 对象被设置为在 Verticle 成功完成其 `start` 或 `stop` 生命周期时完成。 如果出现错误，`Promise` 对象将失败，并出现描述错误的异常，并且 Verticle 部署失败。
...
> **💡提示:** 使用接受回调来通知错误的异步方法变体是一种很好的健壮性实践，例如清单 2.7 中的 `listen` 方法。 如果它允许我减少代码示例的冗长，我不会在本书的其余部分总是这样做。
...
请注意，虽然这听起来是分层的，但 Vert.x 并没有正式的"父/子" Verticle 概念。
...
最后但同样重要的是，`Deployer` verticle 本身可以从主方法和类部署，如清单 2.10 所示。 运行此示例会产生类似于以下清单中的日志条目。
...
这给我们上了一堂有趣的课: 一个Verticle总是使用同一个事件循环线程时，事件循环线程被多个Verticle共享。这种设计导致运行应用程序的线程数量是可预测的。
...
- Worker Verticle 只能在给定时间由被个工作线程访问。

简单来说，就像event-loop verticles一样，worker verticles也是单线程的，但与event-loop verticles不同，线程可能并不总是相同的。
...
清单2.14 显示了一个使用两个实例部署worker verticle对象的示例。每10秒，代码就会阻塞8秒。运行这个示例将产生与清单2.15类似的输出。如您所见，不同的工作线程被用于连续的事件。
...
下面的清单显示了运行清单2.16中的代码的一些示例输出。正如您所看到的，执行被卸载到工作线程，但结果处理仍然发生在事件循环中。
...
>
> **译者注:** 也就是说如果`ordered `是false,就可以并发执行`executeBlocking`了!
